...
from valid.Validation import ValidError, RepoError
...
class ServiceClient(object):
    ...
    def removeClient(self, clientId, params):
        ...
        self.__repoClients.removeElement(clientId)
...
class ServiceRental(object):
    ...
    def rentBook(self, rentalId, bookId, clientId, rentedDate, dueDate, undo = True):
        ...
        if self.__repo.searchElement1(new_rent) == -1:
            new_rent = Rental(rentalId, bookId, clientId, rentedDate, dueDate, 1)
            ...
            i = self.__repo.searchElement1(new_rent)
            if self.__repo.rentElement(new_rent, i):
                number = self.__repo.numberOfRents(bookId)
                number = int(number)
                new_rent = Rental(rentalId, bookId, clientId, rentedDate, dueDate, number)
                self.__repo.addElement(new_rent)
                if undo == True:
                    self.__undoService.addNewOperation(self.__repo.rentBook, self.__repo.removeElementUndo, [new_rent, rentalId], index )
            else:
