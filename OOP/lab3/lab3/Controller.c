...
void listIntendedUseIngredients(char** finalList, char* intendedUse, SubstancesController *controller) {
	listIntendedUseSubstances(finalList, "intendedUse", intendedUse, controller->substancesRepo);
}

void listStateIngredients(char** finalList, char* state, SubstancesController *controller) {
	listIntendedUseSubstances(finalList, "state", state, controller->substancesRepo);
}

void deallocateController(SubstancesController* controller) {
	deallocateRepository(controller->substancesRepo);
	deallocateUndoRedo(controller->undoController);
	free(controller);
}
...
SubstancesController* createController() {
	...
	newController->substancesRepo = createRepository();
...
void listMaximumPotencyValueIngredientsDescending(char** finalList, int maximumPotencyValue, SubstancesController *controller) {
	sortMaximumPotencyValueDescending(finalList, maximumPotencyValue, controller->substancesRepo);
}
...
void undoOperation(char** finalList, SubstancesController *controller) {
	if (controller->undoController->lengthOfUndoList == 0)
		...
		//deleteMemory(oldSubstance);
		//deallocateUndoOperation(newOperation);
	}
}
