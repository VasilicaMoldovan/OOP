...
void TaskRepository::openRepoFile() {
	//ShellExecuteA(NULL, NULL, "chrome.exe", this->pathToFile.c_str(), NULL, SW_SHOWMAXIMIZED);
	//ShellExecute(nullptr, "open", this->pathToFile.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
	system(this->pathToFile.c_str());
}
...
InMemoryRepo::InMemoryRepo(TaskRepository *repo)
{
	this->pathToFile = repo->getPath();
	this->repositoryVector = repo->getAllTasks();
}
...
void TaskRepository::deletePosition(int index)
{
	this->repositoryVector.erase(this->repositoryVector.begin() + index);
}
