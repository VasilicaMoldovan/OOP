	...
	: QMainWindow(parent), movieController(controller)
{
	...
	connect(ui.showWatchlistButton, SIGNAL(released()), this, SLOT(handleUserlistButton()));
...
void Lab12::handleUserlistButton()
{
	ui.watchlistData->clear();
...
void Lab12::populateUserDatabase()
{
	for (Movie &movie : this->movieController.getWatchlist())
	{
		ui.watchlistData->addItem(QString::fromStdString(movie.toStringMovie()));
	}
}
